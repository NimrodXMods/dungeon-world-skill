...
metadata:
  ...
  last-modified-by-model: Anthropic Claude Sonnet 5 (claude-sonnet-5)
  updated: "2026-08-05"
...
- **rulebook-digest** - a hierarchical (L0/L1/L2) digest of the full 410-page core rulebook, built as an `advanced-digest`. `L0-index.md` tracks chapter-by-chapter coverage; `L1-digest.md` holds one paragraph per section plus ~60 atomic L2 facts (`F-NNN`) and full catalogs for anything genuinely precise a paraphrase would lose - including the complete bestiary (9 chapters, ~130 monsters/NPCs), all 8 playbooks, and the full Wizard/Cleric spell lists. Several durable findings were also folded directly into the other reference files above (corrections are noted inline where that happened) - read `L0-index.md` first for an overview of what's where. For anything that would require L3, instead read specific pages referenced in `L1-digest.md` from `source/core-rulebook-full-text.txt` as described below.
- **rulebook-digest/source/core-rulebook-full-text.txt** - the complete `pdftotext -layout` conversion of the core rulebook (~100k words, ~670KB), with a `===== PAGE N =====` marker inserted before each page so it's `grep`/`view`-able by exact page number without needing the original PDF re-uploaded. Every "re-fetch page X" note in `L1-digest.md` refers to this file - use `grep -n "===== PAGE 91 ====="` (or a range) to jump straight to it. Only ever read the specific pages you need, never the whole file at once.
...
2. **Read the matching L1 section(s) first.** Most questions resolve here. Only descend to L2 if the L1 paragraph doesn't contain the specific figure, name, or claim being asked about, and only open L3 if exact wording (not just the fact) matters.
