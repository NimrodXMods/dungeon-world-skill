# GM narration essays (warm)

**When to read:** when entering [SKILL-1a-create.md](SKILL-1a-create.md) (new campaign
fiction) or [SKILL-2-main-loop.md](SKILL-2-main-loop.md) (play), if not already loaded
this session. **Not** for pure rules-only / about.

**Always-on short rules** (style labels + hard bullets) live in [SKILL.md](SKILL.md)
under "How to Write the Game" / narration constitution — do not skip those.

Style lane details (Pulpy / Grim / Formal) and `style_voice` are in SKILL.md; this
file expands theory-of-mind, description depth, and related edge cases.
