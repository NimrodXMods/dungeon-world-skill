...
TEMPLATE_SCHEMA_PAIRS = [
    ...
    ("environment_template.yaml", "environment.schema.yaml"),
]
...
def check_moves_asset():
    """assets/moves.json must carry the rulebook move text the dashboard shows.

    Generated by tools/extract_moves.py from the vendored rulebook XML. The
    parser walks a layout-driven document, so a refresh of the source can
    quietly produce a structurally valid file with nothing in it - which the
    dashboard renders as moves with no descriptions rather than as an error.
    These are shape and floor checks, not a diff against the book.
    """
    path = ASSETS / "moves.json"
    if not path.is_file():
        fail(rel(path), "missing - run tools/extract_moves.py")
        return
    ...
    # The book has 8 basic and 13 special moves; require at least most of them
    # so a parser that silently matched almost nothing fails here.
    for key, floor in (("basic", 8), ("special", 12)):
        entries = data.get(key)
        if not isinstance(entries, list) or len(entries) < floor:
            fail(
                rel(path),
                "{} holds {} moves, expected at least {} - the extractor probably "
                "stopped matching the source".format(
                    key, len(entries) if isinstance(entries, list) else "no", floor
                ),
            )
            return

    classes = data.get("classes")
    if not isinstance(classes, dict) or len(classes) < 8:
        fail(rel(path), "expected the 8 core classes, got {}".format(
            sorted(classes) if isinstance(classes, dict) else classes))
        return

    for name, buckets in sorted(classes.items()):
        for bucket in ("starting", "advanced"):
            entries = buckets.get(bucket) if isinstance(buckets, dict) else None
            if not isinstance(entries, list) or not entries:
                fail(rel(path), "{} has no {} moves".format(name, bucket))
                return

    # Every move needs text, or the dashboard shows a bare name and the asset
    # is not earning the ~120KB it costs.
    empty = []
    for group in ("basic", "special"):
        empty += [m.get("name") for m in data[group] if not m.get("blocks")]
    for name, buckets in classes.items():
        for bucket in ("starting", "advanced"):
            empty += ["{}/{}".format(name, m.get("name"))
                      for m in buckets[bucket] if not m.get("blocks")]
    if empty:
        fail(rel(path), "{} move(s) carry no description text, e.g. {}".format(
            len(empty), ", ".join(str(n) for n in empty[:5])))


def check_dashboard_hook():
    """Prove yamledit actually finds scripts/yamledit.yaml and runs dashboard.py.

    This is the only check that can. A hook that fails - or a config yamledit
    never located because of a typo in its filename or keys - is *by design*
    completely silent: hooks.warn() goes to stderr and never changes yamledit's
    exit code, precisely so a broken dashboard cannot make the model believe its
    edit failed. So nothing else here would notice the feature had stopped
    working. It has to be exercised for real.

    The spoiler canary at the end is the assertion that matters. Everything else
    about the dashboard is cosmetic; rendering gmsecret content into a page the
    player is looking at is the one way this feature can do harm.
    """
    config = SCRIPTS / "yamledit.yaml"
    script = SCRIPTS / "dashboard.py"
    template = ASSETS / "yaml_templates" / "character_template.yaml"
    page_template = ASSETS / "html_templates" / "dashboard_template.html"
    for path in (config, script, template, page_template):
        if not path.is_file():
            fail(rel(path), "missing - the dashboard hook cannot work without it")
            return

    canary = "CANARY_GMSECRET_MUST_NOT_APPEAR_8Q4Z"

    with tempfile.TemporaryDirectory() as tmp:
        work = Path(tmp)
        (work / "canary_warrior.yaml").write_bytes(template.read_bytes())
        # Deliberately a near-miss on the character glob: same directory, same
        # extension, and it must still never be opened.
        (work / "canary_gmsecret.yaml").write_text(
            "campaign_slug: canary\nsecret: {}\n".format(canary), encoding="utf-8"
        )

        result = run(
            "scripts/yamledit.pyz", str(work / "canary_warrior.yaml"),
            "--schema", "assets/yaml_schemas/character.schema.yaml",
            stdin="name -> Canary\nhp.current -> 7\n",
        )
        if result.returncode != 0:
            fail(rel(config), "yamledit could not set up the fixture ({})".format(
                result.stderr.strip()))
            return

        page = work / "DW_Dashboard.html"
        if not page.is_file():
            fail(
                rel(config),
                "a real yamledit write did not produce DW_Dashboard.html - the hook "
                "config is not being found or dashboard.py failed (yamledit stderr: {})".format(
                    result.stderr.strip() or "silent, as a failing hook always is"
                ),
            )
            return
        ...
        start, end = "<!--DW-DATA-START-->", "<!--DW-DATA-END-->"

        # Read the carrier the way a BROWSER does, not the way it is convenient
        # to slice. The content of a <script> element is raw text, so the HTML
        # comment markers are not comments there - they are literal characters
        # handed to the page as part of textContent. A validator that slices
        # between the markers itself cannot see that, and will happily pass a
        # page whose own JSON.parse throws and silently renders empty.
        carrier = re.search(
            r'<script[^>]*id="dw-data"[^>]*>([\s\S]*?)</script>', html)
        if not carrier:
            fail(rel(page_template), "generated page has no #dw-data script element")
            return

        # Counted inside the carrier only: the renderer legitimately mentions
        # both markers as string literals, so counting across the whole page
        # would never be 1/1.
        raw = carrier.group(1)
        if raw.count(start) != 1 or raw.count(end) != 1:
            fail(
                rel(page_template),
                "data carrier holds {}/{} markers, expected 1/1 - regeneration "
                "must splice between them, not append".format(
                    raw.count(start), raw.count(end)
                ),
            )
            return

        blob = raw[raw.index(start) + len(start):raw.rindex(end)]
        try:
            payload = json.loads(blob)
        except ValueError as exc:
            fail(rel(script), "injected block is not valid JSON ({})".format(exc))
            return

        # ...and the page's own JS must perform that same strip. If it parses
        # textContent directly it throws on the markers, and the catch renders
        # the "no campaign loaded" placeholder - a total failure that looks
        # exactly like a working page with an empty campaign.
        renderer = html.rsplit("</script>", 2)[-2] if html.count("</script>") >= 2 else ""
        if start not in renderer or end not in renderer:
            fail(
                rel(page_template),
                "the page's renderer does not strip the {} / {} markers before "
                "JSON.parse - inside a <script> they are raw text, so parsing "
                "would throw and the page would render empty".format(start, end),
            )

        names = [c.get("name") for c in payload.get("characters", [])]
        if "Canary" not in names:
            fail(rel(script), "character sheet did not reach the page (got {})".format(names))

        # The move reference is campaign-independent, so it must be on every
        # page - including one built from a bare template with no moves listed.
        reference = payload.get("reference") or {}
        if not reference.get("basic") or not reference.get("special"):
            fail(
                rel(script),
                "the basic/special move reference did not reach the page - "
                "assets/moves.json is missing or was not loaded",
            )

        # A second write must overwrite, not accumulate.
        result = run(
            "scripts/yamledit.pyz", str(work / "canary_warrior.yaml"),
            "--schema", "assets/yaml_schemas/character.schema.yaml",
            stdin="hp.current -> 9\n",
        ...
        if result.returncode == 0:
            again = re.search(
                r'<script[^>]*id="dw-data"[^>]*>([\s\S]*?)</script>',
                page.read_text(encoding="utf-8"),
            )
            if not again or again.group(1).count(start) != 1:
                fail(rel(script), "a second run appended a data block instead of replacing it")

        # The one that matters.
        if canary in page.read_text(encoding="utf-8"):
            fail(
                rel(script),
                "gmsecret content reached the player-facing dashboard - *_gmsecret.yaml "
                "must be excluded by filename before any read",
            )
...
def main(argv=None):
    ...
    if QUICK:
        ...
        skipped.append("dashboard hook end-to-end (check_dashboard_hook)")
    else:
        check_session_roundtrip()
        check_dashboard_hook()
